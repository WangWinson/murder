...
var typeTable = {ConflictFreeReplicatedDataType: CRDT, CRDT: CRDT, GenericCRDT: CRDT};
...
CRDT.defineOperationBehavior = function (name, behavior) {
  return function replay(params, operation, origin, localOnly) {
    ...
    debug.log(this.type + '..' + name + '-behavior-check: ' + operation);
    ...
    return Promise.all(promises).then(function () {
      debug.info(this.type + '..op:"' + name + '") resolved.', this.version, '\n',
        operation.toString() + '\n',
        this.state);
...
CRDT.prototype.emitContinuation = function (operation, origin) {
  ...
  if (!exists) { debug.warn(new Error('No local event found for continuation')); }
...
CRDT.prototype.invoke = function (operation, origin, localOnly) {
  ...
  console.log(operation, this.type);
...
CRDT.prototype.append = function (operation, origin, localOnly) {
  ...
  console.log(!localOnly, operation.method === 'sync', (!operation.params || !operation.originalParams));
  ...
  console.log(isDuplicate, localHash);
  ...
  return new Promise(function (resolve, reject) {
    sourcePromises.then(function () {
      ...
      if (this.relays.length === 0) {
        if (operation.method === 'sync') { this.broadcastSyncResponse(operation, origin); }
        else { this.emitContinuation(operation, origin); }
      }
...
CRDT.prototype.compress = function (callback) {
  ...
  return this.throttle('compress',
    ...
    function () {
      ...
      var compressedSync = this.newOperation('sync', {merge: this.state});
...
CRDT.prototype.resolveState = function (callback) {
  ...
  return this.throttle('resolve',
    ...
    function () {
      ...
      debug.log(this.type + '..resolveState-ready');
      console.log(this.tombestone, this.version, this.state, this.lastOperationTime);

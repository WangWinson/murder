...
CRDT.defineOperationBehavior = function (name, behavior) {
  return function (params, operation, origin, noBroadcast) {
    ...
    var continuationPromise = /*!noBroadcast &&*/ this.newContinuation(operation, origin);
    ...
    if (this.isInvalidOperation(operation)) { this.resolveState(); }

    try {
      // Operation behavior methods may return a promise.
      behaviorPromise = behavior.call(this, params, operation, origin, noBroadcast);
...
CRDT.prototype.newContinuation = function (operation, origin) {
  debug.log(this.type + '..newContinuation:', '', '\n', operation.toString());
  ...
  return new Promise(function (resolve, reject) {
    ...
    var listener = function () {
      debug.log(this.type + '..newContinuation-listener:', '', '\n', operation.toString());
      if (rejected) {
      ...
      if (count >= relayCount) {
        clearTimeout(timer);
        resolve(this);
        debug.log(this.type + '..newContinuation-resolve:', '', '\n', operation.toString());
...
CRDT.prototype.manualAppendSync = false;

CRDT.prototype.append = function (operation, origin, isManual) {
  debug.log(this.type + '..append:', '', '\n', operation.toString());

  // No need to store sync operations with no params.
  if (operation.method === 'sync' && (!operation.originalParams || (!isManual && this.manualAppendSync)))
...
CRDT.prototype.broadcast = function (operation, origin) {
  ...
  var sourcePromises = Promise.all(this.sources.map(function (source) {
    if (origin !== source) { return source.deliver(operation, origin).then(
      console.log.bind(console, 'source:deliver-resolve'),
      console.error.bind(console, 'source:deliver-reject')); }
  }.bind(this))).then(
    console.log.bind(console, 'sourcePromises-resolve'),
    console.error.bind(console, 'sourcePromises-reject'));

  console.log('BD-SR', this.sources.length, this.relays.length);
...
CRDT.prototype.compress = function (callback) {
  ...
  return this.throttle('compress',
    ...
    function () {
      ...
      this.sources.map(function (source) { source.compress(this); }.bind(this));
    });
};

CRDT.prototype.resolveDelayMin = 100;
CRDT.prototype.resolveDelayMax = 10000;
...
CRDT.prototype.resolveState = function () {
  ...
  log.forEach(function (operation) {
...
CRDT.prototype.isInvalidOperation = function (operation) {
  debug.log(this.type + '..isInvalidOperation');
...
CRDT.defineOperation('sync', function (params, operation, origin) {
  ...
  if (params) {
    ...
    if (params.state || params.merge) { this.compress(); }
  }
